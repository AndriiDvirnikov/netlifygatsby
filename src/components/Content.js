import React from 'react'


export default function Content() {
    return (
        <div className ="content">
            <div className ="conten-bg-gray">
                <div className="conteiner">
                    <div className="hold-card">
                        <div className = "cart-blog">
                            <div className ="hold-img-blog">
                                <img src ="/blog-final_03.jpg"/>
                                <div className ="categorry-blog red">
                                    <p>Trips</p>
                                </div>
                            </div>
                            <div className ="sub-text-blog">
                                <p>Donec elementum dui semper, pretium dui quis, pretium nisl. Nunc quis ornare odio.</p>
                            </div>
                            <div className="author">
                                <p className="date-slider">14 septemvber 2016</p>
                                <p className="author-news">John Doe</p>
                            </div>
                        </div>
                        <div className = "cart-blog">
                            <div className ="hold-img-blog">
                                <img src ="/blog-final_05.jpg"/>
                                <div className ="categorry-blog black">
                                    <p>Tech</p>
                                </div>
                            </div>
                            <div className ="sub-text-blog">
                                <p>Donec elementum dui semper, pretium dui quis, pretium nisl. Nunc quis ornare odio.</p>
                            </div>
                            <div className="author">
                                <p className="date-slider">14 septemvber 2016</p>
                                <p className="author-news">John Doe</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div className ="conteiner">
                <div className="hold-solo-block">
                    <img src="/blog-final13.jpg"/>

                    <div className="text-blog">
                        <h1>Proin congue elit fringilla</h1>
                        <p>sodales tellus interdum fermentum nulla. Aliquam vitae arcu condimentum,
                            consectetur diam sed, ultrices urna.
                        </p>
                    </div>
                    <div className="hold-slider-info">
                        <div className = "slider-info">
                            <div className="categorry blue">
                                <p>People</p>
                            </div>
                            <div className ="author">
                                <p className="date-slider">11 septemvber 2016</p>
                                <p className="author-news">John Doe</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div className="conteiner">
                <div className="hold-blog-mini">
                    <div className="hold-blog-inner">
                        <div className="cart-blog">
                            <div className ="hold-img-blog">
                                <img src ="/blog-final_10.jpg"/>
                                <div className ="categorry-blog green">
                                    <p>Nature</p>
                                </div>
                            </div>
                            <div className ="sub-text-blog">
                                    <p>Donec elementum dui semper, pretium dui quis, pretium nisl.</p>
                            </div>
                            <div className="author">
                                <p className="date-slider">11 septemvber 2016</p>
                                <p className="author-news">John Doe</p>
                            </div>
                        </div>
                        <div className="cart-blog">
                            <div className ="hold-img-blog">
                                <img src ="/blog-final_12.jpg"/>
                                <div className ="categorry-blog blue">
                                    <p>People</p>
                                </div>
                            </div>
                            <div className ="sub-text-blog">
                                    <p>Donec elementum dui semper, pretium dui quis, pretium nisl.</p>
                            </div>
                            <div className="author">
                                <p className="date-slider">11 septemvber 2016</p>
                                <p className="author-news">John Doe</p>
                            </div>
                        </div>
                        <div className="cart-blog">
                            <div className ="hold-img-blog">
                                <img src ="/blog-final_14.jpg"/>
                                <div className ="categorry-blog black">
                                    <p>Tech</p>
                                </div>
                            </div>
                            <div className ="sub-text-blog">
                                    <p>Donec elementum dui semper, pretium dui quis, pretium nisl.</p>
                            </div>
                            <div className="author">
                                <p className="date-slider">11 septemvber 2016</p>
                                <p className="author-news">John Doe</p>
                            </div>
                        </div>
                    </div>

                    <div className="hold-blog-inner">
                        <div className="cart-blog">
                            <div className ="hold-img-blog">
                                <img src ="/blog-final_19.jpg"/>
                                <div className ="categorry-blog red">
                                    <p>Trips</p>
                                </div>
                            </div>
                            <div className ="sub-text-blog">
                                    <p>Donec elementum dui semper, pretium dui quis, pretium nisl.</p>
                            </div>
                            <div className="author">
                                <p className="date-slider">11 septemvber 2016</p>
                                <p className="author-news">John Doe</p>
                            </div>
                        </div>
                        <div className="cart-blog">
                            <div className ="hold-img-blog">
                                <img src ="/blog-final_20.jpg"/>
                                <div className ="categorry-blog red">
                                    <p>Trips</p>
                                </div>
                            </div>
                            <div className ="sub-text-blog">
                                    <p>Donec elementum dui semper, pretium dui quis, pretium nisl.</p>
                            </div>
                            <div className="author">
                                <p className="date-slider">11 septemvber 2016</p>
                                <p className="author-news">John Doe</p>
                            </div>
                        </div>
                        <div className="cart-blog">
                            <div className ="hold-img-blog">
                                <img src ="/blog-final_21.jpg"/>
                                <div className ="categorry-blog purple">
                                    <p>Fashion</p>
                                </div>
                            </div>
                            <div className ="sub-text-blog">
                                    <p>Donec elementum dui semper, pretium dui quis, pretium nisl.</p>
                            </div>
                            <div className="author">
                                <p className="date-slider">11 septemvber 2016</p>
                                <p className="author-news">John Doe</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div className ="conten-bg-gray">
                <div className="conteiner">
                    <div className="hold-card">
                        <div className = "cart-blog">
                            <div className ="hold-img-blog">
                                <img src ="/blog-final_03.jpg"/>
                                <div className ="categorry-blog red">
                                    <p>Trips</p>
                                </div>
                            </div>
                            <div className ="sub-text-blog">
                                <p>Donec elementum dui semper, pretium dui quis, pretium nisl. Nunc quis ornare odio.</p>
                            </div>
                            <div className="author">
                                <p className="date-slider">14 septemvber 2016</p>
                                <p className="author-news">John Doe</p>
                            </div>
                        </div>
                        <div className = "cart-blog">
                            <div className ="hold-img-blog">
                                <img src ="/blog-final_05.jpg"/>
                                <div className ="categorry-blog black">
                                    <p>Tech</p>
                                </div>
                            </div>
                            <div className ="sub-text-blog">
                                <p>Donec elementum dui semper, pretium dui quis, pretium nisl. Nunc quis ornare odio.</p>
                            </div>
                            <div className="author">
                                <p className="date-slider">14 septemvber 2016</p>
                                <p className="author-news">John Doe</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            
        </div>
    
            
    )
}
